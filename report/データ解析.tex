...
\subsection{筋電データ処理}
筋電計から取得したデータには、1$\sim$4Hzの通過領域をもつバンドパスフィルタをバターワースフィルタを用いてかけ、
%筋電計から取得した
データに混じる高周波のノイズを除去した。
%筋活動の変化に注目するため、pythonで1$\sim$40Hzをバターワースフィルタを用いたバンドパスフィルタによって取り出した。
フィルタをかけると位相のずれが生じるため、出力信号の時間を反転して、同じ特性のフィルタを再度通して、全体の時間のずれを直した。
%全体の時間のずれを直すためゼロ位相ずれフィルタをかけた。
次に、筋活動量の時間変化を、(\ref{s2})の式を用いて算出した。(\ref{s2})では$t$が時間、$\delta Tが窓区間、$E(t)$が筋活動度を示す。以上の処理を行うプログラムを\ref{prog1}にまとめた。
